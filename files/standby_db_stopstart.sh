...
srvctl status database -d $dbms|cut -d ' ' -f 7 > $FILE1
   ...
   do
   >${scriptdir}/standby_db_stop_${instclistNode}.wil.csc.local_prepatch.sh
   >${scriptdir}/standby_db_start_${instclistNode}.wil.csc.local_postpatch.sh
   done
done
...
echo "srvctl stop database -db ${dbms}" >> ${scriptdir}/standby_db_stop_${INSTHOSTNAME}.wil.csc.local_prepatch.sh
echo "srvctl start database -db ${dbms}" >> ${scriptdir}/standby_db_start_${INSTHOSTNAME}.wil.csc.local_postpatch.sh
done < $FILE2
done
fi
